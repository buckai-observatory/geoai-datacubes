#!/bin/bash
#SBATCH --job-name=geoai-smoke-fetch-hls-l30
...
# ---------------------------------------------------------------------------
# Smoke test: HLS Landsat leg via PC.
#
# Runs either as a regular bash script (local) or as a SLURM job:
#   bash   smoke-tests/fetch_hls-l30.sh
#   sbatch smoke-tests/fetch_hls-l30.sh
#
...
# to smoke-tests/logs/fetch_hls-l30.json so the run history can be committed.
# ---------------------------------------------------------------------------

# shellcheck source=_common.sh
source "$(dirname "${BASH_SOURCE[0]}")/_common.sh"

python smoke-tests/_run_fetch.py "HLS_L30"
