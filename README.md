...
>
> - **NISAR L-band SAR over Helheim Glacier** (v0.2 preview — this branch only) — cutting-edge showcase of the new `earthdata` provider fetching **NISAR** L-band SAR (public archive opened 2026-07-20, first proper open L-band SAR archive since ALOS PALSAR-1) alongside Sentinel-1 C-band, Copernicus DEM, and Sentinel-2 optical, fused into one UTM cube over Helheim Glacier's calving front in southeast Greenland.
>   <a href="https://colab.research.google.com/github/buckai-observatory/geoai-datacubes/blob/feature/earth-engine-provider/notebooks/05_nisar_helheim_datacube.ipynb" target="_blank" rel="noopener noreferrer"><img src="https://colab.research.google.com/assets/colab-badge.svg" alt="Open in Colab"/></a>
>   <!-- BRANCH-PREVIEW: swap `feature/earth-engine-provider` -> `main` at merge time -->
>
...
### 5. NISAR L-band SAR + Sentinel-1 + DEM over Helheim Glacier (v0.2 preview — this branch only)

[`notebooks/05_nisar_helheim_datacube.ipynb`](notebooks/05_nisar_helheim_datacube.ipynb)
<a href="https://colab.research.google.com/github/buckai-observatory/geoai-datacubes/blob/feature/earth-engine-provider/notebooks/05_nisar_helheim_datacube.ipynb" target="_blank" rel="noopener noreferrer"><img src="https://colab.research.google.com/assets/colab-badge.svg" alt="Open in Colab"/></a>
<!-- BRANCH-PREVIEW: swap `feature/earth-engine-provider` -> `main` at merge time -->

Cutting-edge showcase of the new `earthdata` data provider added on this branch. Fetches **NISAR L-band SAR** (`NISAR_L2_GCOV_PROVISIONAL_V1`, NASA-ISRO mission whose public data archive opened just **7 weeks before this notebook was written** — 2026-07-20 — the first proper open L-band SAR archive since ALOS PALSAR-1 shut down in 2011) via the new `earthdata` provider (NASA CMR + Earthdata Login), alongside **Sentinel-1 C-band SAR**, **Copernicus DEM**, and optional **Sentinel-2** optical over Helheim Glacier's calving front in southeast Greenland (66.4°N, -38.1°W, one of Earth's fastest-flowing tidewater glaciers). Fuses everything into a single multi-band UTM cube on a common grid, then does a direct **L-band vs C-band comparison at the same polarisation (HH)** side-by-side over the ice tongue — the two disagree in a physically meaningful way, and the notebook explains why. Includes the standard three-mode auth pattern (`EARTHDATA_USERNAME`/`EARTHDATA_PASSWORD` env vars, `~/.netrc`, interactive; legacy `EDL_USERNAME`/`EDL_PASSWORD` names also accepted) mirroring notebook 04's EE story, and Colab-friendly bootstrap that installs `earthaccess` + `h5py` + reads two additional userdata secrets. See [`docs/providers/earthdata.md`](docs/providers/earthdata.md) for the auth walkthrough. Live progress on this branch: [PR #18](https://github.com/buckai-observatory/geoai-datacubes/pull/18).
